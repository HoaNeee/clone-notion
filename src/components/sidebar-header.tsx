...
const DropdownSidebarUser = () => {
  ...
  const handleChangeWorkspace = useCallback(
    async (workspace_id: number, is_guest?: boolean, force_change = false) => {
      ...
      try {
        ...
        if (!is_guest && user?.id) {
          setLastWorkspaceInLocalStorage({ workspace_id, user_id: user.id });
        }
      } catch (error) {
  ...
  const handleChangePrevWorkspace = useCallback(async () => {
    try {
      ...
      if (res.last_note) {
        router.push(`/${res.last_note.slug}`);
      }
    } catch (error) {
  ...
  const renderTrigger = useCallback(() => {
    const isGuest =
      (currentWorkspace?.is_guest || isGuestInWorkspace) && currentWorkspace;
    ...
    return isGuest ? (
      ...
      <DropdownMenuTrigger asChild>
        <SidebarMenuButton className="group/item relative flex justify-between cursor-pointer">
          <div className="size-6 bg-neutral-200 text-neutral-500 flex items-center justify-center text-xs font-bold uppercase rounded-sm">
            {nameFallback}
          </div>
      ...
      <SidebarMenu>
        ...
        <SidebarMenuItem>
          <DialogSearch
            ...
            setOpenPopoverTrash={setOpenPopoverTrash}
          />
        </SidebarMenuItem>
        <SidebarMenuItem>
          <Link href={"/home"}>
