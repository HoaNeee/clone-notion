				...
				<div className="py-4 flex flex-col gap-5">
					<div className="relative">
						...
						{isFocusedInput && inputEmail.trim().length ? (
							<div
								className="absolute top-11 text-sm rounded p-1 left-0 w-full bg-white shadow"
								onPointerDown={(e) => e.preventDefault()}
							>
								<div
									className="p-1 bg-neutral-200/60 cursor-pointer rounded-sm text-sm w-full flex	items-center gap-2"
					...
					<div>
						...
						<div className="border rounded border-neutral-200 min-h-15 flex gap-2 flex-wrap p-2 bg-neutral-50">
							{emailList.length === 0 ? (
					...
					<div className="w-full">
						<Label className="block text-xs mb-1 font-medium text-neutral-500">
							Select role
						</Label>
						<Select
							defaultValue="admin"
...
const DropdownSidebarUser = () => {
	...
	const { currentWorkspace, workspaces, workspacesBeInvited } = useWorkspace();
	...
	const handleChangeWorkspace = useCallback(
		async (workspace: TWorkspace) => {
			...
			try {
				...
				is_guest =
					workspace.is_guest ||
					workspacesBeInvited.some((w) => w.id === workspace.id);
				const res = (await get(
					`/workspaces/detail/${workspace.id}?is_guest=${is_guest ? 1 : 0}`
				)) as {
					workspace: TWorkspace;
					defaultNote: TNote | null;
				};

				const { defaultNote } = res;
				...
				if (workspace) {
					localStorage.setItem("last_workspace_id", workspace.id.toString());
				}
			} catch (error) {
				logAction("Error changing workspace:", error);
			}
		},
		[router, workspacesBeInvited, currentWorkspace]
	);
