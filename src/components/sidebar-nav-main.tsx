...
const NavItem = ({
  ...
  const active = slug && item.type === "note" && slug === (item as TNote).slug;
        ...
        >
          ...
          <CollapsibleContent>
            <div className="text-neutral-500 px-2 text-sm">
              <NavList data={folder.children || []} rootFolder={rootFolder} />
            </div>
          </CollapsibleContent>
        </Collapsible>
      </>
...
const NavListContainer = ({ rootFolder }: { rootFolder: TFolder }) => {
...
const ItemRootLabel = ({
  ...
  rootFolder: TFolder;
  handleDrop: (item: TFolder | TNote, next: TFolder) => Promise<void>;
  data: (TFolder | TNote)[];
  openCollapsible: boolean;
  setOpenCollapsible: Dispatch<SetStateAction<boolean>>;
}) => {
  const [openDropdownAddNew, setOpenDropdownAddNew] = useState(false);
  ...
  return (
    <Collapsible open={openCollapsible} defaultOpen={true} draggable={false}>
      ...
      <CollapsibleContent className="h-full">
        <NavListContainer rootFolder={rootFolder} />
      </CollapsibleContent>
    </Collapsible>
  );
};
...
const SidebarNavMain = () => {
  ...
  const [openCollapsibleRoot, setOpenCollapsibleRoot] = useState(true);
  ...
  const [{ isOver, canDrop }, drop] = useDrop(
    () => ({
      ...
      drop(itemDragging) {
        if (rootFolder) {
          handleDropItem(itemDragging as TFolder | TNote, rootFolder);
        }
      },
      collect(monitor) {
        return {
  ...
  return (
    <SidebarGroup className="list-container" ref={sidebarRef}>
      <ItemRootLabel
        ...
        openCollapsible={openCollapsibleRoot}
        setOpenCollapsible={setOpenCollapsibleRoot}
      />
      {openCollapsibleRoot && (
        <div
          className="min-h-5"
