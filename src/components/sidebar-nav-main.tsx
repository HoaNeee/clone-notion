...
function getTitle(title: string, type: TFolder["type"]) {
	if (!title) {
		if (type === "folder") {
			return "New Folder";
		}
...
function solveMetaDialogConfirm(current: TFolder | TNote, next: TFolder) {
	const current_is_in_teamspace = !!current.is_in_teamspace;
	const next_is_in_teamspace = !!next.is_in_teamspace;

	if (current_is_in_teamspace === next_is_in_teamspace) {
		return null;
	}
	...
	let title = "",
		description = "";
	if (current_is_in_teamspace !== next_is_in_teamspace) {
		if (current_is_in_teamspace) {
			if (current.type === "folder") {
				title = "Are you sure you want to move this folder to become private?";
				description =
					"Moving this folder to private will remove all its contents from the teamspace. Are you sure you want to proceed?";
			} else {
				title = "Are you sure you want to move this file to become private?";
				description =
					"This action will move the file to a private folder and restrict access. Are you sure you want to proceed?";
			}
		} else if (next_is_in_teamspace) {
			if (next.type === "folder") {
				title = "Are you sure you want to move this folder to the teamspace?";
				description =
					"Moving this folder to the teamspace will make all its contents accessible to team members. Are you sure you want to proceed?";
			} else {
				title = "Are you sure you want to move this file to the teamspace?";
				description =
					"This action will move the file to the teamspace and allow team members to access it. Are you sure you want to proceed?";
			}
		}
	} else return null;
...
const DialogConfirmDragAndDrop = ({
	open,
	setOpen,
	title,
	description,
	...
	return (
		<AlertDialogConfirm
			...
			setOpen={setOpen}
			title={
				<p className="text-center leading-5.5">{title || "Are you sure?"}</p>
			}
			description={
				description ||
				"This action will move the file to a private folder and restrict access."
			}
			onOk={onOk}
		/>
	);
...
const DraggableElementContainer = ({
	children,
	item,
	className,
	onDrop,
...
const NavItemFolder = ({
	...
	const [openConfirmDragAndDrop, setOpenConfirmDragAndDrop] = useState(false);
	...
	const openDialogDNDRef = useRef<boolean>(false);
	...
	const onDrop = useCallback(
		async (dragging_item: TFolder | TNote, next_folder: TFolder) => {
			//draggingItem -> dragged draggingItem, folder -> drop to folder (default is folder)
			let payload = {
				...dragging_item,
			} as Partial<TFolder | TNote>;

			if (dragging_item.type === "folder") {
				payload = {
					...payload,
					parent_id: next_folder.id,
				};
			}

			if (dragging_item.type === "note") {
				payload = {
			...
			try {
				const current_is_in_teamspace = !!dragging_item.is_in_teamspace;
				const next_is_in_teamspace = !!next_folder.is_in_teamspace;

				if (
					current_is_in_teamspace !== next_is_in_teamspace &&
					(!openConfirmDragAndDrop || !openDialogDNDRef.current)
				) {
					const meta = solveMetaDialogConfirm(dragging_item, next_folder);

					if (meta) {
						setDialogMeta(meta);
						setDataDND({ item: dragging_item, next_item: next_folder });
						openDialogDNDRef.current = true;
						setOpenConfirmDragAndDrop(true);
						return;
					}
				}

				if (!openCollapsibleFolder && !existIdsGlobal.has(next_folder.id)) {
					fetchData(next_folder.id, next_folder.is_in_teamspace);
				}
				setOpenCollapsibleFolder(true);
				...
				await onUpdate({
					...
					is_in_teamspace: dragging_item.is_in_teamspace,
				});

				setDataDND(null);
				setOpenConfirmDragAndDrop(false);
				openDialogDNDRef.current = false;
			} catch (error) {
		...
		[openCollapsibleFolder, fetchData, openConfirmDragAndDrop, onUpdate]
	);
	...
	return (
		<>
			...
			<DialogConfirmDragAndDrop
				open={openConfirmDragAndDrop}
				setOpen={setOpenConfirmDragAndDrop}
				title={dialogMeta.title}
				description={dialogMeta.description}
				onOk={() => {
					onDrop(dataDND!.item, dataDND!.next_item);
				}}
			/>
		</>
	);
...
const NavList = ({
	...
	return <p className="text-sm text-center">Empty Folder</p>;
};
...
const ListPrivate = ({
	...
	const [openDropdownAddNew, setOpenDropdownAddNew] = useState(false);
	const [openDialogConfirmDragAndDrop, setOpenDialogConfirmDragAndDrop] =
		useState(false);
	const [dialogMeta, setDialogMeta] = useState<{
		title: string;
		description: string;
	...
	const [{ isOver, canDrop }, drop] = useDrop(
		() => ({
			...
			drop(itemDragging) {
				if (rootFolder) {
					onDrop(itemDragging as TFolder | TNote, rootFolder);
				}
			},
			collect(monitor) {
				return {
	...
	const onDrop = useCallback(
		async (item: TFolder | TNote, next: TFolder) => {
			...
			if (item.type === "folder") {
				payload = {
					...payload,
					parent_id: next.id,
					is_in_teamspace: next.is_in_teamspace,
				};
			}
			...
			try {
				const current_is_in_teamspace = !!item.is_in_teamspace;
				const next_is_in_teamspace = !!next.is_in_teamspace;

				if (
					current_is_in_teamspace !== next_is_in_teamspace &&
					(!openDialogConfirmDragAndDrop || !openDialogDNDRef.current)
				) {
					const meta = solveMetaDialogConfirm(item, next);

					if (meta) {
						setDialogMeta(meta);
						setOpenDialogConfirmDragAndDrop(true);
						setDataDND({ item, next_item: next });
						openDialogDNDRef.current = true;
						return;
					}
				}
				...
				setOpenDialogConfirmDragAndDrop(false);
				setDataDND(null);
				openDialogDNDRef.current = false;
			} catch (error) {
		...
		[openDialogConfirmDragAndDrop, onUpdate]
	);
	...
	return (
		<>
			...
			<DialogConfirmDragAndDrop
				open={openDialogConfirmDragAndDrop}
				setOpen={setOpenDialogConfirmDragAndDrop}
				title={dialogMeta.title}
				description={dialogMeta.description}
				onOk={() => {
					onDrop(dataDND!.item, dataDND!.next_item);
