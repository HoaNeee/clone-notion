...
const PopoverSettingIcon = ({
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			>
				<div
					className={`size-17 flex items-center justify-center bg-gray-200 rounded-sm overflow-hidden ${
...
const WorkspaceGeneralSettings = () => {
	...
	const isGuest = isGuestInWorkspace || currentWorkspace.is_guest;
	const disabled = isGuest || currentWorkspace.role !== "admin";

	return (
		<div className="relative flex flex-col w-full h-full overflow-hidden rounded-md">
			<div className="flex flex-col w-full overflow-hidden overflow-y-auto h-full px-10 py-5">
				{isGuest ? null : (
					<SettingMenuGroup label="Workspace Settings">
						<div className="flex flex-col w-full gap-5 py-6 text-sm">
							...
							<div className="flex flex-col gap-2">
								...
								<div className="flex flex-col gap-1">
									<div className="size-20 flex items-center justify-center border rounded-sm overflow-hidden">
										<PopoverSettingIcon
							...
							<SettingMenuItem
								...
								action={
									<Switch
										...
										checked={!!workspaceSetting?.is_profile}
										onCheckedChange={(checked) =>
											updateWorkspaceSetting("is_profile", checked ? 1 : 0)
										}
									/>
								}
								disabled={disabled}
							...
							<SettingMenuItem
								...
								action={
									<Switch
										...
										checked={!!workspaceSetting?.is_hover_card}
										onCheckedChange={(checked) =>
											updateWorkspaceSetting("is_hover_card", checked ? 1 : 0)
										}
									/>
								}
								disabled={disabled}
					...
					<SettingMenu className="inline-flex items-start">
						<div className="flex flex-row gap-2">
							...
							{isGuest ? null : (
								<Button
									variant={"outline"}
									className="border-destructive/80 text-destructive/80 hover:text-destructive/80 text-sm disabled:cursor-not-allowed"
									disabled={disabled}
								>
