...
const WorkspaceContext = ({
	...
	useEffect(() => {
		...
		if (!currentWorkspace && !slug && !isGuestInWorkspace) {
			const lastObjectWorkspace = localStorage.getItem("last_object_workspace");

			if (lastObjectWorkspace) {
				const parse = JSON.parse(lastObjectWorkspace) as {
					workspace_id: number;
					user_id: number;
				};
				if (parse.user_id === user?.id) {
					fn(parse.workspace_id);
					return;
				}
			}
		}
