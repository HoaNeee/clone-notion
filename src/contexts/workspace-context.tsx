...
import { getLastWorkspaceObjectInLocalStorage } from "@/utils/workspace";
...
const WorkspaceContext = ({
  ...
  useEffect(() => {
    ...
    if (!currentWorkspace && !slug && !isGuestInWorkspace) {
      const lastObjectWorkspace = getLastWorkspaceObjectInLocalStorage();

      if (lastObjectWorkspace) {
        if (lastObjectWorkspace.user_id === user?.id) {
          fn(lastObjectWorkspace.workspace_id);
          return;
        }
      }
    }
  }, [currentWorkspace, slug, token, user?.id, isGuestInWorkspace]);
