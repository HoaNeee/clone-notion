...
const FolderContext = ({
	...
	const { currentNote, setCurrentNote, noteFavorites, onFavoriteNote } =
		useNote();
	...
	useEffect(() => {
		existIdsGlobal.clear();
		if (rootFolder) {
			existIdsGlobal.add(rootFolder.id);
		}
		if (rootFolderInTeamspace) {
			existIdsGlobal.add(rootFolderInTeamspace.id);
		}
	...
	const fetchDataTree = useCallback(
		async (folder_id: number, is_in_teamspace?: boolean) => {
			if (existIdsGlobal.has(folder_id)) {
				return;
			}

			existIdsGlobal.add(folder_id);
			try {
				setIsLoading(true);
				const [folders, notes] = await Promise.all([
	...
	const onUpdate = useCallback(
		...
		[router, currentNote, data, dataInTeamspace, slug]
	);
	...
	const onDelete = useCallback(
		async (
			...
			try {
				if (type === "folder") {
					await del(`/folders/move-to-trash/${id}`);
				}
				if (type === "note") {
					if (noteFavorites.find((n) => n.id === id)) {
						const note = noteFavorites.find((n) => n.id === id);
						if (currentWorkspace && note) {
							await onFavoriteNote(note, currentWorkspace.id, "remove");
						}
					}

					const note = data.find(
						(n) => n.id === id && n.type === "note"
					) as TNote;
					if (note && currentNote) {
						const clean_slug = slug ? slug.toLowerCase() : "";
						const clean_note_slug = note.slug ? note.slug.toLowerCase() : "";
					...
					await del(`/notes/move-to-trash/${id}`);
				}
	...
	const createRootFolderAndNoteDefaultInTeamspace = useCallback(
		async (workspace_id: number) => {
			...
			try {
				const res = (await post(
					`/folders/create-root-and-default-note-teamspace?workspace_id=${workspace_id}`
				)) as {
					folder: TFolder;
					note: TNote;
				};
